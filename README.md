# Indonesia Merger Calculator

A browser-based helper tool for calculating mergers in the board game *Indonesia*.

This tool helps players quickly evaluate:

- Starting bids
- Company share payouts
- Projected income

Designed for fast in-game use on desktop or mobile.

---
## 📸 Screenshots

<p align="center">
  <a href="docs/screenshot-light.jpg">
    <img src="docs/screenshot-bidding-light.jpg" width="48%" alt="Light mode">
  </a>
  <a href="docs/screenshot-dark.jpg">
    <img src="docs/screenshot-bidding-dark.jpg" width="48%" alt="Dark mode">
  </a>
</p>

<p align="center">
  <sub>Click either image to view the full screenshot</sub>
</p>

## 🌐 Live Version

Available here:  
https://timurkelman.github.io/indonesia-merge-calculator/

---

## ✨ Features

- Merger bid calculation
- Company share payout calculation
- Projected income calculation
- Mobile-friendly interface
- Dark mode support
- Installable on your phone and works offline — no app store required

---

## 🎲 About the Game

*Indonesia* is a strategy board game designed by Jeroen Doumen and Joris Wiersinga and published by Splotter Spellen.

Official shop page:  
https://www.splottershop.com/products/indonesia

BoardGameGeek entry:  
https://boardgamegeek.com/boardgame/19777/indonesia

All trademarks and game rights belong to their respective owners.

---

## 🙏 Origin & Attribution

This project is based on the original **Indonesia Merger Calculator** published on BoardGameGeek:

https://boardgamegeek.com/filepage/177563/indonesia-merger-calculator

Original author: Chris Burke (aka Eric Torch)  
https://boardgamegeek.com/profile/EricTorch

Credit to the original author for creating the first version of this very useful tool for the community.

This repository is a heavily rewritten and modernized implementation.

While inspired by the original concept, this version is not a direct copy of the source code and is independently implemented.

---

## ⚠ Disclaimer

This is an unofficial fan-made tool.

It is not affiliated with, endorsed by, or associated with Splotter Spellen or the creators of *Indonesia*.

All trademarks and game rights belong to their respective owners.

---

## 📜 License

This project is licensed under the  
**Creative Commons Attribution–NonCommercial 4.0 International (CC BY-NC 4.0)** license.

You may:

- Share
- Adapt
- Modify

But you may **not use this project for commercial purposes**.

See the LICENSE file for full details.

---

## 🍵 Support

If this calculator helped, you can buy me a tea:

https://ko-fi.com/TimurKelman
